function songUpdateHandler() {
  ...
  let song_studentnum = document.getElementById('song_contributor_field').value
...
function bookUpdateHandler(){
    ...
    let book_offset = document.getElementById('book_offset_field').value
    let book_pages = document.getElementById('book_pages_field').value
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      */
      ...
      songDetailsDiv.innerHTML += `</div>`
      songDetailsDiv.innerHTML += `<div class="details_wrapper">`
      songDetailsDiv.innerHTML += `<div class="input_lable">CONTRIBUTOR: <input class="details" type="text" id="song_contributor_field" value="${response.contributor}"/></div>`
      songDetailsDiv.innerHTML += `</div>`
      songDetailsDiv.innerHTML += `<div class="details_wrapper">`
...
function bookSelectHandler(book_id){
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        songDetailsDiv.innerHTML += `<div class="details_wrapper">`
        songDetailsDiv.innerHTML += `<div class="input_lable">BOOKCODE: <input class="details" type="text" id="book_bookcode_field" value="${response.bookcode}" readonly/></div>`
        songDetailsDiv.innerHTML += `</div>`
        songDetailsDiv.innerHTML += `<div class="details_wrapper">`
        ...
        songDetailsDiv.innerHTML += `</div>`
        songDetailsDiv.innerHTML += `<div class="details_wrapper">`
        songDetailsDiv.innerHTML += `<div class="input_lable">PAGE OFFSET: <input class="details" type="text" id="book_offset_field" value="${response.page_offset}"/></div>`
        songDetailsDiv.innerHTML += `</div>`
        songDetailsDiv.innerHTML += `<div class="details_wrapper">`
        songDetailsDiv.innerHTML += `<div class="input_lable">PAGES: <input class="details" type="text" id="book_pages_field" value="${response.num_pages}"/></div>`
        songDetailsDiv.innerHTML += `</div>`
        songDetailsDiv.innerHTML += `<div class="details_wrapper">`
      ...
      xhr.onreadystatechange = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
          ...
          for (let book of response.books) {
            ...
            <font color="blue">${book.bookcode}</font></li>`
          }
