...
exports.api_books = function(request, response) {
  ...
  let sql = "SELECT rowid as id, bookcode FROM bookcodes WHERE bookcode LIKE ? LIMIT 30"
  let title = '%' //sql match anything character
  if (urlObj.query['title']) {
    title = `%${urlObj.query['title']}%`
    console.log("finding title: " + urlObj.query['title'])
...
exports.api_bookDetails = function(request, response){
    ...
    let sql = "SELECT rowid, bookcode, title, format, filename, page_offset, num_pages FROM bookcodes WHERE bookcode=?"
...
/*
  ...
  NEED TO MODIFY THE UPDATE SONGS TO UPDATE BOOKS
*/
